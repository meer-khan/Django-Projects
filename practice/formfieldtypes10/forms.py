...
class StudentRegistration(forms.Form):
    ...
    price = forms.DecimalField(
        min_value=5, max_value=40, max_digits=5, decimal_places=2
    )
    ...
    description = forms.CharField(
        widget=forms.Textarea(attrs={"rows": 3, "cols": "50"}), strip=True
    )
    feedback = forms.CharField(
        widget=forms.TextInput(attrs={"class": "Some Class", "id": "uniqueID"}),
        min_length=3,
    )
