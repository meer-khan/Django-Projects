            ...
            if extractedDirPath == None:
                ...
                if os.path.isdir(extractedDirPath):
                    ...
                    if os.path.isdir(newDir):
                        ...
                        raise Exception(
                            "No such Directory exists to extract the Files!"
                        )
                else:
...
def is_file_empty(file_path, content):
    ...
    return (
        False
        if os.stat(file_path).st_size == 0 and os.path.getsize(file_path) == 0
        else (False if len(content) <= 30 else True)
    )


def parsing_all_files_and_folders(extractedFilesDirectory):
    fileNamesNPaths = [
        {"path": os.path.join(root, filename), "fileName": filename}
...
def writing_to_csv(userDict, csvDirectory):
    ...
    try:
        if os.path.isdir(csvDirectory):
            newCSVDirectory = os.path.join(
                csvDirectory,
                "ComplyVantage_Date_%d_%d_%d_Time_%d_%d_%d"
...
guess = Guess()
startTime = time.time()
parsingResult = parsing_all_files_and_folders(
    r"D:\CVtoPUSH\Github\GitHubPOC\Test Cases\hybridTest"
)
